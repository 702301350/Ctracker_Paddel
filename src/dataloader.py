...
class CSVDataset(paddle.io.Dataset):
    ...
    def __init__(self, root_path, train_file, class_list, transform=None):
        ...
        try:
            ...
            raise_from(ValueError('invalid CSV class file: {}: {}'.format(
        ...
        try:
            with self._open_for_csv(self.train_file) as file:
                self.image_data = self._read_annotations(csv.reader(file,
                                                                    delimiter=','), self.classes)
        except ValueError as e:
            raise_from(ValueError('invalid CSV annotations file: {}: {}'.
                                  format(self.train_file, e)), None)
        self.image_names = list(self.image_data.keys())
        self.name2video_frames = dict()
        self.image_name_prefix = list()
        for image_name in self.image_names:
            self.image_name_prefix.append(image_name[0:-len(image_name.
                                                            split('/')[-1].split('_')[-1])])
        self.image_name_prefix = set(self.image_name_prefix)
        print('total vedio count: {}'.format(len(self.image_name_prefix)))
        for image_name in self.image_names:
            cur_prefix = image_name[0:-len(image_name.split('/')[-1].split(
                '_')[-1])]
            if cur_prefix not in self.name2video_frames:
                self.name2video_frames[cur_prefix] = 1
            else:
                self.name2video_frames[cur_prefix] = self.name2video_frames[
                                                         cur_prefix] + 1
    ...
    def _get_random_surroud_name(self, image_name, max_diff=3, ignore_equal
    =True, pos_only=True):
        suffix_name = image_name.split('/')[-1].split('_')[-1]
        ...
        while True:
            ...
            if ignore_equal:
                if next_index == cur_index:
                    continue
            break
        return prefix + '{0:06}.'.format(next_index) + suffix_name.split('.')[
            -1]
        ...
        for line, row in enumerate(csv_reader):
            ...
            try:
                ...
                raise_from(ValueError(
                    "line {}: format should be 'class_name,class_id'".
                    format(line)), None)
            class_id = self._parse(class_id, int,
                                   'line {}: malformed class ID: {{}}'.format(line))
            if class_name in result:
                raise ValueError("line {}: duplicate class name: '{}'".
                                 format(line, class_name))
    ...
    def __getitem__(self, idx):
        while True:
            try:
                ...
                if tuple(annot.shape)[0] < 1 or tuple(annot_next.shape)[0] < 1:
        ...
        if np.random.rand() < 0.5:
            sample = {'img': img, 'annot': annot, 'img_next': img_next,
                      'annot_next': annot_next}
        else:
            sample = {'img': img_next, 'annot': annot_next, 'img_next': img,
                      'annot_next': annot}
        if self.transform:
    ...
    def _read_annotations(self, csv_reader, classes):
        ...
        for line, row in enumerate(csv_reader):
            ...
            try:
                ...
                raise_from(ValueError(
                    "line {}: format should be 'img_file,obj_id,x1,y1,x2,y2,class_name' or 'img_file,,,,,'"
                    .format(line)), None)
            ...
            if (x1, y1, x2, y2, class_name) == ('', '', '', '', ''):
                continue
            x1 = self._parse(float(x1), int, 'line {}: malformed x1: {{}}'.
                             format(line))
            y1 = self._parse(float(y1), int, 'line {}: malformed y1: {{}}'.
                             format(line))
            x2 = self._parse(float(x2), int, 'line {}: malformed x2: {{}}'.
                             format(line))
            y2 = self._parse(float(y2), int, 'line {}: malformed y2: {{}}'.
                             format(line))
            if x2 <= x1:
                raise ValueError('line {}: x2 ({}) must be higher than x1 ({})'
                                 .format(line, x2, x1))
            if y2 <= y1:
                raise ValueError('line {}: y2 ({}) must be higher than y1 ({})'
                                 .format(line, y2, y1))
            if class_name not in classes:
                raise ValueError(
                    "line {}: unknown class name: '{}' (classes: {})".
                    format(line, class_name, classes))
            result[img_file].append({'x1': x1, 'x2': x2, 'y1': y1, 'y2': y2,
                                     'class': class_name, 'obj_id': obj_id})
        return result
...
def collater(data):
    ...
    widths = [int(tuple(s.shape)[1]) for s in imgs]
    heights = [int(tuple(s.shape)[0]) for s in imgs]
    batch_size = len(imgs)
    max_width = np.array(widths).max()
    max_height = np.array(heights).max()
    padded_imgs = paddle.zeros(shape=[batch_size, max_height, max_width, 3])
    padded_imgs_next = paddle.zeros(shape=[batch_size, max_height,
                                           max_width, 3])
    for i in range(batch_size):
        img = imgs[i]
        padded_imgs[i, :int(tuple(img.shape)[0]), :int(tuple(img.shape)[1]), :
        ] = img
        img_next = imgs_next[i]
        padded_imgs_next[i, :int(tuple(img_next.shape)[0]), :int(tuple(
            img_next.shape)[1]), :] = img_next
    max_num_annots = max(tuple(annot.shape)[0] for annot in annots)
    max_num_annots_next = max(tuple(annot.shape)[0] for annot in annots_next)
    max_num_annots = max(max_num_annots, max_num_annots_next)
    if max_num_annots > 0:
        annot_padded = paddle.ones(shape=(len(annots), max_num_annots, 6)) * -1
        if max_num_annots > 0:
            for idx, annot in enumerate(annots):
                if tuple(annot.shape)[0] > 0:
                    annot_padded[idx, :tuple(annot.shape)[0], :] = annot
    else:
        annot_padded = paddle.ones(shape=(len(annots), 1, 6)) * -1
    if max_num_annots > 0:
        annot_padded_next = paddle.ones(shape=(len(annots_next),
                                               max_num_annots, 6)) * -1
        if max_num_annots > 0:
            for idx, annot in enumerate(annots_next):
                if tuple(annot.shape)[0] > 0:
                    annot_padded_next[idx, :tuple(annot.shape)[0], :] = annot
    else:
        annot_padded_next = paddle.ones(shape=(len(annots_next), 1, 6)) * -1
    padded_imgs = padded_imgs.transpose(perm=[0, 3, 1, 2])
    padded_imgs_next = padded_imgs_next.transpose(perm=[0, 3, 1, 2])
    return {'img': padded_imgs, 'annot': annot_padded, 'img_next':
        padded_imgs_next, 'annot_next': annot_padded_next}
...
class Resizer(object):
    ...
    def __call__(self, sample, min_side=608, max_side=1024):
        return sample
        image, annots, image_next, annots_next = sample['img'], sample['annot'
        ], sample['img_next'], sample['annot_next']
        rows, cols, cns = tuple(image.shape)
        ...
        image_next = skimage.transform.resize(image_next, (int(round(rows *
                                                                     scale)), int(round(cols * scale))))
        rows, cols, cns = tuple(image.shape)
        pad_w = 32 - rows % 32
        pad_h = 32 - cols % 32
        new_image = np.zeros((rows + pad_w, cols + pad_h, cns)).astype(np.
                                                                       float32)
        new_image[:rows, :cols, :] = image.astype(np.float32)
        new_image_next = np.zeros((rows + pad_w, cols + pad_h, cns)).astype(np
                                                                            .float32)
        new_image_next[:rows, :cols, :] = image_next.astype(np.float32)
        annots[:, :4] *= scale
        annots_next[:, :4] *= scale
        return {'img': paddle.to_tensor(data=new_image), 'annot': paddle.
        to_tensor(data=annots), 'img_next': paddle.to_tensor(data=
                                                             new_image_next), 'annot_next': paddle.to_tensor(data=
                                                                                                             annots_next), 'scale': scale}
...
class Augmenter(object):
    ...
    def __call__(self, sample, flip_x=0.5):
        if np.random.rand() < flip_x:
            ...
            rows, cols, _ = tuple(image.shape)
            rows_next, cols_next, _ = tuple(image_next.shape)
            ...
            sample = {'img': image, 'annot': annots, 'img_next': image_next,
                      'annot_next': annots_next}
        return sample
...
class Normalizer(object):
    ...
    def __call__(self, sample):
        image, annots = sample['img'], sample['annot']
        image_next, annots_next = sample['img_next'], sample['annot_next']
        return {'img': paddle.to_tensor(data=(image.astype(np.float32) /
                                              255.0 - self.mean) / self.std), 'annot': paddle.to_tensor(data=
                                                                                                        annots), 'img_next': paddle.to_tensor(data=(image_next.astype(
            np.float32) / 255.0 - self.mean) / self.std), 'annot_next':
                    paddle.to_tensor(data=annots_next)}
...
class UnNormalizer(object):
    ...
    def __call__(self, tensor):
        ...
        for t, m, s in zip(tensor, self.mean, self.std):
            t.multiply_(y=paddle.to_tensor(s)).add_(y=paddle.to_tensor(m))
        return tensor
...
class PhotometricDistort(object):
    ...
    def __call__(self, sample):
        ...
        return {'img': image, 'annot': annots, 'img_next': image_next,
                'annot_next': annots_next}
...
class RandomSampleCrop(object):
    ...
    def __call__(self, sample):
        ...
        height, width, _ = tuple(image.shape)
        ...
        for _ in range(20):
            ...
            rect = np.array([int(left), int(top), int(left + crop_size),
                             int(top + crop_size)])
            ...
            if np.random.uniform(0, 1) > 0.75:
                height, width, depth = tuple(image.shape)
                ...
                expand_image = np.zeros((int(height * expand_ratio), int(
                    width * expand_ratio), depth), dtype=image.dtype)
                expand_image[:, :, :] = np.array([[RGB_MEAN]]) * 255.0
                expand_image[int(top):int(top + height), int(left):int(left +
                                                                       width)] = image
                image = expand_image
                annots[:, :2] += int(left), int(top)
                annots[:, 2:4] += int(left), int(top)
                expand_next_image = np.zeros((int(height * expand_ratio),
                                              int(width * expand_ratio), depth), dtype=image_next.dtype)
                expand_next_image[:, :, :] = np.array([[RGB_MEAN]]) * 255.0
                expand_next_image[int(top):int(top + height), int(left):int
                (left + width)] = image_next
                image_next = expand_next_image
                annots_next[:, :2] += int(left), int(top)
                annots_next[:, 2:4] += int(left), int(top)
            image = (255.0 * skimage.transform.resize(image, (target_size,
                                                              target_size))).astype(np.uint8)
            image_next = (255.0 * skimage.transform.resize(image_next, (
            ...
            return {'img': image, 'annot': annots, 'img_next': image_next,
                    'annot_next': annots_next}
        if not crop_success:
            image = (255.0 * skimage.transform.resize(image, (height // 2,
                                                              width // 2))).astype(np.uint8)
            image_next = (255.0 * skimage.transform.resize(image_next, (
                height // 2, width // 2))).astype(np.uint8)
            annots[:, :4] *= 0.5
            annots_next[:, :4] *= 0.5
        return {'img': image, 'annot': annots, 'img_next': image_next,
                'annot_next': annots_next}
...
class AspectRatioBasedSampler(paddle.io.Sampler):
