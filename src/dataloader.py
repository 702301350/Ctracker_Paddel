        ...
        for line, row in enumerate(csv_reader):
            ...
            try:
                ...
                raise_from(ValueError(
                    'line {}: format should be \'img_file,obj_id,x1,y1,x2,y2,class_name\' or \'img_file,,,,,\''.format(
                        line)), None)
...
def collater(data):
    ...
    padded_imgs = paddle.zeros(batch_size, max_height, max_width)
    padded_imgs_next = paddle.zeros(batch_size, max_height, max_width)
    ...
    if max_num_annots > 0:
        annot_padded = paddle.ones((len(annots), max_num_annots, 6)) * -1
        if max_num_annots > 0:
            for idx, annot in enumerate(annots):
                if annot.shape[0] > 0:
                    annot_padded[idx, :annot.shape[0], :] = annot
    else:
        annot_padded = paddle.ones((len(annots), 1, 6)) * -1

    if max_num_annots > 0:
        annot_padded_next = paddle.ones((len(annots_next), max_num_annots, 6)) * -1
        ...
        annot_padded_next = paddle.ones((len(annots_next), 1, 6)) * -1
...
class Resizer(object):
    ...
    def __call__(self, sample, min_side=608, max_side=1024):
        ...
        return {'img': torch.from_numpy(new_image), 'annot': torch.from_numpy(annots),
                'img_next': torch.from_numpy(new_image_next), 'annot_next': torch.from_numpy(annots_next),
                'scale': scale}
...
class Normalizer(object):
    ...
    def __call__(self, sample):
        ...
        return {'img': paddle.to_tensor((image.astype(np.float32) / 255.0 - self.mean) / self.std),
                'annot': paddle.to_tensor(annots),
                'img_next': paddle.to_tensor((image_next.astype(np.float32) / 255.0 - self.mean) / self.std),
                'annot_next': paddle.to_tensor(annots_next)}
    ...
    def __call__(self, sample):
        ...
        for _ in range(20):
            ...
            if np.random.uniform(0, 1) > 0.75:
                ...
                expand_image = np.zeros((int(height * expand_ratio), int(width * expand_ratio), depth),
                                        dtype=image.dtype)
...
class AspectRatioBasedSampler(Sampler):
    ...
    def group_images(self):
        ...
        order.sort(key=lambda x: self.data_source.image_aspect_ratio(
            x))
