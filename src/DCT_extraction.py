...
class MultiSpectralAttentionLayer(paddle.nn.Layer):

    def __init__(self, channel, dct_h, dct_w, reduction=16, freq_sel_method
    ='top16'):
...
class MultiSpectralDCTLayer(paddle.nn.Layer):
    """
    ...
    def forward(self, x):
        assert len(tuple(x.shape)) == 4, 'x must been 4 dimensions, but got ' + str(len(tuple(x.shape)))
        x = x * self.weight
